...
export default function BlogList({
  ...
  const filteredPosts = sectionFilter ? postList.filter((p) => sectionFilter?.includes(p.section)) : postList;
  const tagFilter = useTagFilter(filteredPosts);
  const postsToRender = tagFiltering ? tagFilter.filteredItems : filteredPosts;

  return (
    <div className={cn("page-grid", className)}>
      {sidebar && (
        <Card className="bg-background p-4 col-span-3 md:col-span-2 row-span-full prose prose-char
          ...
          {tagFiltering && (
            <TagFilter
              ...
              tagCounts={tagFilter.tagCounts}
              tags={tagFilter.visibleTags}
            />
          )}
        </Card>
      )}
      ...
      <div className={`col-span-3 grid gap-4 ${sidebar ? "md:col-span-6 md:grid-cols-2" : "md:col-span-8 md:grid-cols-2"}`}>
        {postsToRender.map((post, index) => {
          const isLastOdd = postsToRender.length % 2 !== 0 && index === postsToRender.length - 1;
